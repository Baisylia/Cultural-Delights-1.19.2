...
public class FermenterRecipeCategory implements IRecipeCategory<FermenterRecipe> {
    public final static ResourceLocation UID = new ResourceLocation(CulturalDelights.MOD_ID, "fermenting");
    ...
    @Override
    public void draw(FermenterRecipe recipe, IRecipeSlotsView recipeSlotsView, PoseStack poseStack, double mouseX, double mouseY) {
        IDrawableAnimated arrow = getArrow(recipe);
        arrow.draw(poseStack, 63, 19);
        drawCookTime(recipe, poseStack, 45);
    }

    protected void drawCookTime(FermenterRecipe recipe, PoseStack poseStack, int y) {
    ...
    protected IDrawableAnimated getArrow(FermenterRecipe recipe) {
        int cookTime = recipe.getCookTime();
        if (cookTime <= 0) {
    ...
    public RecipeType<FermenterRecipe> getRecipeType() {
        return JEICulturalDelightsPlugin.FERMENTING_TYPE;
    }
